...
bool ofxGizmo::mouseMoved(int x, int y) {
    if (gizmo && isVisible()) gizmo->OnMouseMove( mouseOffset.x + x, mouseOffset.y + y );
    return _bInteracting;
}
...
bool ofxGizmo::mousePressed(int x, int y, int button) {
    if (gizmo && isVisible()) {
        if (gizmo->OnMouseDown( mouseOffset.x + x, mouseOffset.y + y )) {
            _bInteracting = true;
            return true;
        }
    }
...
bool ofxGizmo::mouseReleased(int x, int y, int button) {
    if (gizmo && isVisible()) gizmo->OnMouseUp( mouseOffset.x + x, mouseOffset.y + y );
